...
public class CreateSignalErrorTest {
    ...
    private static Stream<Arguments> provideStringsForBodyCreateSignal() {
        return Stream.of(
            Arguments.of(null, new BigDecimal("10.0"), new BigDecimal("4.0"), UUID.randomUUID(), "XS0587031096", "L01+00000SPB", 1),
//            Arguments.of(CreateSignalRequest.ActionEnum.SELL, "2000356465", null, 4, UUID.randomUUID(), "XS0587031096", "L01+00000SPB", 1),
            ...
            Arguments.of(CreateSignalRequest.ActionEnum.SELL, new BigDecimal("10.0"), new BigDecimal("4.0"), null, "XS0587031096", "L01+00000SPB", 1),
            Arguments.of(CreateSignalRequest.ActionEnum.SELL, new BigDecimal("10.0"), new BigDecimal("4.0"), UUID.randomUUID(), null, "L01+00000SPB", 1),
            Arguments.of(CreateSignalRequest.ActionEnum.SELL, new BigDecimal("10.0"), new BigDecimal("4.0"), UUID.randomUUID(), "XS0587031096", null, 1),
            Arguments.of(CreateSignalRequest.ActionEnum.SELL, new BigDecimal("10.0"), new BigDecimal("4.0"), UUID.randomUUID(), "XS0587031096", "L01+00000SPB", null)
        );
    }
    ...
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C656034(CreateSignalRequest.ActionEnum action, BigDecimal price, BigDecimal quantityRequest,
...
//        getExchangePosition(instrument.tickerAAPL, instrument.tradingClearingAccountAAPL, ExchangePosition.ExchangeEnum.SPB, true, 1000);
    ...
    private static Stream<Arguments> provideStringsFlags() {
        return Stream.of(
            Arguments.of(CreateSignalRequest.ActionEnum.BUY),
            Arguments.of(CreateSignalRequest.ActionEnum.SELL)
    ...
    @SneakyThrows
    @ParameterizedTest
    @MethodSource("provideStringsFlags")
    @AllureId("1641339")
    @DisplayName("C1641339.CreateSignal.Создание торгового сигнала ведущим, action = buy/sell, Проверка на доступность создания сигнала при флаге strategy.buy_enabled/sell_enabled = false")
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C1641339(CreateSignalRequest.ActionEnum action) {
        ...
        steps.createClientWithContractAndStrategy(SIEBEL_ID, investIdMaster, null, contractIdMaster, null, ContractState.untracked,
            strategyId, steps.getTitleStrategy(), description, StrategyCurrency.rub, ru.qa.tinkoff.tracking.entities.enums.StrategyRiskProfile.aggressive,
            StrategyStatus.active, 0, LocalDateTime.now(), 1, "0.2", "0.04", false, new BigDecimal(58.00), "TEST", "TEST11",false,false, null);
        OffsetDateTime utc = OffsetDateTime.now(ZoneOffset.UTC);
        Date date = Date.from(utc.toInstant());
        List<MasterPortfolio.Position> positionMasterList = steps.masterOnePositions(date, instrument.tickerSBER,
        ...
        String errorMessage = jsonObject.getString("errorMessage");
        assertThat("код ошибки не равно", errorCode, is("Error"));
        assertThat("Сообщение об ошибке не равно", errorMessage, is("Данный сигнал заблокирован"));
    }
...
//        getExchangePosition(ticker, tradingClearingAccount, ExchangePosition.ExchangeEnum.SPB, true, 1000);
    ...
    void C1456532() {
        ...
        adminSteps.updateExchangePosition(instrument.tickerAAPL, instrument.tradingClearingAccountAAPL, Exchange.SPB,
            true, 1000, orderQuantityListAll(4, "default", 0, "main_trading",
                0, "additional_liquidity", 0, "primary"), false);
        //формируем тело запроса метода CreateSignal
        CreateSignalRequest request = createSignalRequest(CreateSignalRequest.ActionEnum.BUY,
            price, quantityRequest, strategyId, instrument.tickerAAPL, instrument.tradingClearingAccountAAPL, version);
        // вызываем метод CreateSignal
    ...
    void C1439792() {
        ...
        //устанавливаем значения limit для проверяемого инструмента
        adminSteps.updateExchangePosition(instrument.tickerALFAperp, instrument.tradingClearingAccountALFAperp, Exchange.MOEX,
            true, 22845, orderQuantityList(10, "default"), false);
        //формируем тело запроса метода CreateSignal
        CreateSignalRequest request = createSignalRequest(CreateSignalRequest.ActionEnum.BUY,
            price, quantityRequest, strategyId, instrument.tickerALFAperp, instrument.tradingClearingAccountALFAperp, version);
        // вызываем метод CreateSignal
    ...
    @Test
    @AllureId("1439793")
    @DisplayName("1439793 Объем заявки. Продажа. period = default. tailOrderQuantity > limit")
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C1439793() {
        BigDecimal price = new BigDecimal("5341.8");
    ...
    @Test
    @AllureId("1807503")
    @DisplayName("1807503 Объем заявки. Покупка. period = null. dynamicsLimit = false")
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C1807503() {
        ...
        Response createSignal = signalApiCreator.get().createSignal()
            ...
            .respSpec(spec -> spec.expectStatusCode(500))
            .execute(response -> response);
        ErrorResponse errorResponse = createSignal.as(ErrorResponse.class);
        //проверяем ответ ошибки
        adminSteps.checkErrors(errorResponse, "Error", "Сервис временно недоступен");
    }
    ...
    @Test
    @AllureId("1803801")
    @DisplayName("1803801 Объем заявки. Покупка. period = default. tailOrderQuantity > limit. dynamicsLimit = true.")
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C1803801() {
        BigDecimal price = new BigDecimal("135");
        ...
        //устанавливаем значения limit для проверяемого инструмента
        adminSteps.updateExchangePosition(instrument.tickerSBER, instrument.tradingClearingAccountSBER, Exchange.MOEX,
            true, 115000, orderQuantityList(52, "default"), true);
        //рассчёты
    ...
    @Test
    @AllureId("1803792")
    @DisplayName("1803792 Объем заявки. Продажа. period = default. tailOrderQuantity > limit. dynamics = true")
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C1803792() {
        BigDecimal price = new BigDecimal("135");
        ...
        String tailValue = "15000000";
        double money = 100000.0;
        double quantityPosMasterPortfolio = 300.0;
        LocalDateTime now = LocalDateTime.now(ZoneId.of("Z"));
        log.info("Получаем локальное время: {}", now);
        //получаем данные по клиенту master в api сервиса счетов
        GetBrokerAccountsResponse resAccountMaster = steps.getBrokerAccounts(SIEBEL_ID);
    ...
    @Test
    @AllureId("1888691")
    @DisplayName("1888691 Не найден торговый статус позиции в в кэше exchangePositionTradingStatusCache")
    @Subfeature("Альтернативные сценарии")
    @Description("Метод для создания торгового сигнала ведущим на увеличение/уменьшение соответствующей позиции в портфелях его ведомых.")
    void C1888691() {
        double money = 1500.0;
